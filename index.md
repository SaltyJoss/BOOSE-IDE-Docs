
<div align="left">
	<img src="favicon/BOOSE-API-DOCS.png" width="480" height="96">
</div>

 <br> 
 <br> 

## **INDEX PAGE**

> :memo: **Note:** The classes below are automatically generated from the BOOSECoreApp XML documentation.

### [Adapters](api/BOOSECoreApp.Core.Adapters.yml)
- [CanvasGraphicsAdapter](api/BOOSECoreApp.Core.Adapters.CanvasGraphicsAdapter.yml)
- [SoftVariable](api/BOOSECoreApp.Core.Adapters.SoftVariable.yml)
- [VariableAdapterApp](api/BOOSECoreApp.Core.Adapters.VariableAdapterApp.yml)

### [Base](api/BOOSECoreApp.Core.Base.yml)
- [CallFrame](api/BOOSECoreApp.Core.Base.CallFrame.yml)
- [CanvasApp](api/BOOSECoreApp.Core.Base.CanvasApp.yml)
- [CommandApp](api/BOOSECoreApp.Core.Base.CommandApp.yml)
- [CommandFactoryApp](api/BOOSECoreApp.Core.Base.CommandFactoryApp.yml)
- [CommandWrapper](api/BOOSECoreApp.Core.Base.CommandWrapper.yml)
- [EvaluationApp](api/BOOSECoreApp.Core.Base.EvaluationApp.yml)
- [ParsedLine](api/BOOSECoreApp.Core.Base.ParsedLine.yml)
- [ParsedLineType](api/BOOSECoreApp.Core.Base.ParsedLineType.yml)]
- [ParserApp](api/BOOSECoreApp.Core.Base.ParserApp.yml)
- [StoredProgramApp](api/BOOSECoreApp.Core.Base.StoredProgramApp.yml)
- [UINotificationEvenetHandler](api/BOOSECoreApp.Core.Base.UINotificationEvenetHandler.yml)
- [UINotificationType](api/BOOSECoreApp.Core.Base.UINotificationType.yml)
- [VariableAppBase](api/BOOSECoreApp.Core.Base.VariableAppBase.yml)
- [WrapperApp](api/BOOSECoreApp.Core.Base.WrapperApp.yml)

### [Commands](api/BOOSECoreApp.Core.Commands.yml)
- [AboutApp](api/BOOSECoreApp.Core.Commands.AboutApp.yml)
- [BackgroundColourApp](api/BOOSECoreApp.Core.Commands.BackgroundColourApp.yml)
- [CircleApp](api/BOOSECoreApp.Core.Commands.CircleApp.yml)
- [DrawToApp](api/BOOSECoreApp.Core.Commands.DrawToApp.yml)
- [MoveToApp](api/BOOSECoreApp.Core.Commands.MoveToApp.yml)
- [RectApp](api/BOOSECoreApp.Core.Commands.RectApp.yml)
- [PenColourApp](api/BOOSECoreApp.Core.Commands.PenColourApp.yml)
- [SquareApp](api/BOOSECoreApp.Core.Commands.SquareApp.yml)
- [SysInfoApp](api/BOOSECoreApp.Core.Commands.SysInfoApp.yml)]
- [TriApp](api/BOOSECoreApp.Core.Commands.TriApp.yml)
- [WriteApp](api/BOOSECoreApp.Core.Commands.WriteApp.yml)

### [Interface](api/BOOSECoreApp.Core.Interfaces.yml)
- [IArrayApp](api/BOOSECoreApp.Core.Interfaces.IArrayApp.yml)]
- [ICanvasProvider](api/BOOSECoreApp.Core.Interfaces.ICanvasProvider.yml)
- [ICommandApp](api/BOOSECoreApp.Core.Interfaces.ICommandApp.yml)
- [IGraphicAdapter](api/BOOSECoreApp.Core.Interfaces.IGraphicAdapter.yml)
- [IStoredProgramApp](api/BOOSECoreApp.Core.Interfaces.IStoredProgramApp.yml)

### [Variables](api/BOOSECoreApp.Core.Variables.yml)
- [IntApp](api/BOOSECoreApp.Core.Variables.IntApp.yml)
- [RealApp](api/BOOSECoreApp.Core.Variables.RealApp.yml)
- [BooleanApp](api/BOOSECoreApp.Core.Variables.BooleanApp.yml)
- [ArrayElementType](api/BOOSECoreApp.Core.Variables.ArrayElementType.yml)
- [ArrayApp](api/BOOSECoreApp.Core.Variables.ArrayApp.yml)
- [PeekApp](api/BOOSECoreApp.Core.Variables.PeekApp.yml)
- [PokeApp](api/BOOSECoreApp.Core.Variables.PokeApp.yml)

### [Conditionals](api/BOOSECoreApp.Core.Conditionals.yml)
- [IfApp](api/BOOSECoreApp.Core.Conditionals.IfApp.yml)
- [IfElseApp](api/BOOSECoreApp.Core.Conditionals.IfElseApp.yml)
- [ElseApp](api/BOOSECoreApp.Core.Conditionals.ElseApp.yml)
- [EndIfApp](api/BOOSECoreApp.Core.Conditionals.EndIfApp.yml)
- [ForApp](api/BOOSECoreApp.Core.Conditionals.ForApp.yml)
- [EndForApp](api/BOOSECoreApp.Core.Conditionals.EndForApp.yml)
- [WhileApp](api/BOOSECoreApp.Core.Conditionals.WhileApp.yml)
- [EndWhileApp](api/BOOSECoreApp.Core.Conditionals.EndWhileApp.yml)

### [Methods](api/BOOSECoreApp.Core.Methods.yml)
- [MethodApp](api/BOOSECoreApp.Core.Methods.MethodApp.yml)
- [EndMethodApp](api/BOOSECoreApp.Core.Methods.EndMethodApp.yml)
- [CallMethodApp](api/BOOSECoreApp.Core.Methods.CallMethodApp.yml)

### [Exceptions](api/BOOSECoreApp.Exceptions.yml)
- [BooseSyntaxException](api/BOOSECoreApp.Exceptions.BooseSyntaxException.yml)
- [GlobalExceptionHandler](api/BOOSECoreApp.Exceptions.GlobalExceptionHandler.yml)

## [UI](api/BOOSECoreApp.UI.yml)
- [BOOSEApp](api/BOOSECoreApp.BOOSEApp.yml)
- [InfoAlign](api/BOOSECoreApp.UI.InfoAlign.yml)
- [InfoPos](api/BOOSECoreApp.UI.InfoPos.yml)
- [InfoUI](api/BOOSECoreApp.IU.InfoUI.yml)
- [AboutDisplayUI](api/BOOSECoreApp.UI.AboutDisplayUI.yml)
- [SystemInfoUI](api/BOOSECoreApp.UI.SystemInfoUI.yml)
- [InfoLine](api/BOOSECoreApp.UI.InfoLine.yml)
- [Token](api/BOOSECoreApp.UI.Token.yml)
- [TokenType](api/BOOSECoreApp.UI.TokenType.yml)
- [BooseHighlighting](api/BOOSECoreApp.UI.BooseHighlighting.yml)
- [EditorTheme](api/BOOSECoreApp.UI.EditorTheme.yml)
- [EditorThemes](api/BOOSECoreApp.UI.EditorThemes.yml)
- [UILogger](api/BOOSECoreApp.UI.UILogger.yml)

### [Mocks](api-tests/BOOSECoreApp.Mocks.yml)
- [MockCanvas](api-tests/BOOSECoreApp.Mocks.MockCanvas.yml)
- [MockGraphicAdapter](api-tests/BOOSECoreApp.Mocks.MockGraphicAdapter.yml)]

### [Command Unit Tests](api-tests/BOOSETests.Commands.yml)
- [CanvasAppTests](api-tests/BOOSETests.Commands.CanvasAppTests.yml)
- [CanvasTests](api-tests/BOOSETests.Commands.CanvasTests.yml)
- [GraphicAdapterTests](api-tests/BOOSETests.Commands.GraphicAdapterTests.yml)

### [Variable Unit Tests](api-tests/BOOSETests.Variables.yml)
- [ArrayAppTests](api-tests/BOOSETests.Variables.ArrayAppTests.yml)
- [IntAppTests](api-tests/BOOSETests.Variables.IntAppTests.yml)
- [RealAppTests](api-tests/BOOSETests.Variables.RealAppTests.yml)
- [BooleanAppTests](api-tests/BOOSETests.Variables.BooleanAppTests.yml)
- [PeekAppTests](api-tests/BOOSETests.Variables.PeekAppTests.yml)
- [PokeAppTests](api-tests/BOOSETests.Variables.PokeAppTests.yml)

### [Conditional Unit Tests](api-tests/BOOSETests.Conditionals.yml)
- [IfAppTests](api-tests/BOOSETests.Conditionals.IfAppTests.yml)
- [IfElseAppTests](api-tests/BOOSETests.Conditionals.IfElseAppTests.yml)
- [ElseAppTests](api-tests/BOOSETests.Conditionals.ElseAppTests.yml)
- [EndIfAppTests](api-tests/BOOSETests.Conditionals.EndIfAppTests.yml)
- [ForAppTests](api-tests/BOOSETests.Conditionals.ForAppTests.yml)
- [EndForAppTests](api-tests/BOOSETests.Conditionals.EndForAppTests.yml)
- [WhileAppTests](api-tests/BOOSETests.Conditionals.WhileAppTests.yml)
- [EndWhileAppTests](api-tests/BOOSETests.Conditionals.EndWhileAppTests.yml)

### [Method Unit Tests](api-tests/BOOSETests.Methods.yml)
- [MethodAppTests](api-tests/BOOSETests.Methods.MethodAppTests.yml)
- [EndMethodAppTests](api-tests/BOOSETests.Methods.EndMethodAppTests.yml)
- [CallMethodAppTests](api-tests/BOOSETests.Methods.CallMethodAppTests.yml)

### [Parser Unit Tests](api-tests/BOOSETests.Parser.yml)
- [BOOSEParserTests](api-tests/BOOSETests.Parser.BOOSEParserTests.yml)

### [UI Unit Tests](api-tests/BOOSETests.UI.yml)
- [EditorThemesRegistryTests](api-tests/BOOSETests.UI.EditorThemesRegistryTests.yml)
- [EditorThemesTests](api-tests/BOOSETests.UI.EditorThemesTests.yml)
- [SyntaxHighlightingTests](api-tests/BOOSETests.UI.SyntaxHighlightingTests.yml)
- [UILoggerTests](api-tests/BOOSETests.UI.UILoggerTests.yml)